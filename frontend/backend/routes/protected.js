...
router.post("/follow/:userId", authMiddleware, async (req, res) => {
  try {
    ...
    const followersCount = updatedUser.followers
...
router.get("/following", authMiddleware, async (req, res) => {
  try {
    ...
    const following = await User.find({ followers: req.user.id }).select(
      "-password"
    );
...
router.get("/messages/:roomId", authMiddleware, async (req, res) => {
  try {
    ...
    // Mark messages as read when fetching (messages received by current user)
    await Message.updateMany(
      { roomId, receiverId: req.user.id, read: false },
      { read: true }
    );
...
router.get("/unread-messages-count", authMiddleware, async (req, res) => {
  try {
...
router.get("/chat-users", authMiddleware, async (req, res) => {
  try {
    ...
    // Get unique user IDs that current user has messaged with and track last message time
    const chatUserData = {};
    messages.forEach((msg) => {
      ...
      if (!chatUserData[otherUserId]) {
        chatUserData[otherUserId] = {
    ...
    const unreadMessages = await Message.aggregate([
      {
        $match: {
          ...
          ).default.Types.ObjectId(currentUserId),
          read: false,
        },
      },
      {
    ...
    const chatUsers = await User.find({
      _id: { $in: Object.keys(chatUserData) },
    }).select("-password");

    // Add isFollowing status and unread count
    ...
    const chatUsersWithStatus = chatUsers.map((user) => {
      ...
      const unreadCount = unreadCountMap[user._id.toString()] || 0;
      return {
        ...user.toObject(),
        isFollowing: isFollowing || false,
        unreadCount: unreadCount,
        lastMessageTime: chatUserData[user._id.toString()]?.lastMessageTime,
      };
    });

    // Sort: users with unread messages first, then by last message time
    chatUsersWithStatus.sort((a, b) => {
